...
const App: React.FC = () => {
  const [view, setViewInternal] = useState<AppView>(parseHash().view);
  ...
  useEffect(() => {
    ...
    // Handle specific modes from Firebase Auth links (e.g. resetPassword)
  ...
  if (view === 'verification-pending') return <VerificationPending email={user?.email} onGoToLogin={handleGoToLogin} />;
