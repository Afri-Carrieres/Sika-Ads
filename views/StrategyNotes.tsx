      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {notes.length === 0 && !loading ? (
          <div className="col-span-full py-20 text-center bg-white rounded-[2.5rem] border border-dashed border-gray-200">
            ...
            <button 
              onClick={() => handleOpenModal()}
              className="mt-4 text-[#128686] font-bold text-sm hover:underline"
            >
          ...
          notes.map((note) => (
            <div key={note.id} className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition-all group flex flex-col h-64 relative overflow-hidden">
              <div className="flex justify-between items-start mb-4">
                <div className="bg-[#E7F4F4] p-2 rounded-xl text-[#128686]">
                  <StickyNote size={20} />
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <button 
                    onClick={() => handleOpenModal(note)}
                    className="p-2 text-gray-400 hover:text-[#128686] hover:bg-[#E7F4F4] rounded-xl transition-colors"
                    title="Modifier"
                  >
                    <Edit3 size={16} />
                  </button>
              ...
              <div className="pt-4 border-t border-gray-50 mt-4 flex items-center gap-2 text-[10px] text-gray-300 font-bold uppercase tracking-widest">
                <Calendar size={12} />
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-3">
                <div className="bg-[#D9ECEC] p-2 rounded-xl text-[#128686]">
            ...
            <div className="space-y-6">
              <label className="block space-y-2">
                <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest ml-1">Titre de l'idée</span>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 pl-12 focus:ring-2 focus:ring-[#128686] outline-none font-bold transition-all focus:bg-white text-gray-900 placeholder:text-gray-300"
              ...
              <label className="block space-y-2">
                <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest ml-1">Contenu</span>
                <textarea 
                  value={newContent}
                  onChange={(e) => setNewContent(e.target.value)}
                  className="w-full h-48 bg-gray-50 border border-gray-100 rounded-2xl p-4 focus:ring-2 focus:ring-[#128686] outline-none font-medium transition-all focus:bg-white text-gray-700 resize-none placeholder:text-gray-300"
              ...
              <button 
                ...
                className="w-full bg-[#128686] hover:bg-[#0E6B6B] text-white py-5 rounded-[1.5rem] font-bold uppercase tracking-widest shadow-xl shadow-[#128686]/20 transition-all flex items-center justify-center gap-3 active:scale-95"
              >
                <Save size={18} />
